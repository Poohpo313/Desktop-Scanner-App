...
export function getHelpSupportReplyModalContent(row: HelpSupportReportRow): {
  ...
  if (row.category === "Registration" || row.subject.toLowerCase().includes("registration")) {
    return {
      title: HELP_SUPPORT_OPEN_REPLY_MODAL.title,
      subtitle: HELP_SUPPORT_OPEN_REPLY_MODAL.subtitle,
      statusLabel: "Open",
    };
  }
