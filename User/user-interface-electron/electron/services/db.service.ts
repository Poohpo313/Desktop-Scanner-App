...
const memory = {
  ...
  cloudSync: [] as Record<string, unknown>[],
  activityLogs: [] as Record<string, unknown>[],
};

export async function initDatabase(appPath: string) {
  const url =
    process.env.OFFLINE_DATABASE_URL ??
    process.env.LOCAL_DATABASE_URL ??
    "postgresql://postgres:postgres@localhost:5432/bukolabs_offline";

  const migrationDir = path.join(appPath, "db", "migrations");
  const migrationCandidates = [
    migrationDir,
    path.join(process.resourcesPath, "app", "db", "migrations"),
    path.join(process.resourcesPath, "app.asar.unpacked", "db", "migrations"),
  ];
  const migrationFolder = migrationCandidates.find((candidate) => fs.existsSync(candidate));
  ...
  try {
    ...
    if (migrationFolder) {
      ...
      for (const file of migrationFiles) {
        const migration = fs.readFileSync(path.join(migrationFolder, file), "utf8");
...
export async function query<T = Record<string, unknown>>(
  ...
  if (memoryMode) return memoryQuery(text, params) as T[];
...
function memoryQuery(text: string, params: unknown[]) {
  ...
  if (t.includes("from users") && t.includes("lower(username)")) {
    ...
    return memory.users
      .filter(
        (user) =>
          String(user.username ?? "").toLowerCase() === needle && user.deleted_at == null,
      )
      .map((user) => ({
