...
function dedupeScanDevices(devices) {
    ...
    for (const device of devices) {
        const matchIndex = result.findIndex((entry) => (0, device_match_1.deviceNamesMatch)(entry.name, device.name));
        if (matchIndex === -1) {
            result.push(device);
            continue;
        }
...
async function scanWithWia(deviceName, settings = {}) {
    ...
    try {
        ...
        if (message.includes("Scanner returned no image")) {
            throw new Error(settings.source === "adf"
        ...
        throw new Error(message.replace(/^Command failed:[\s\S]*?Error:\s*/i, "").trim() ||
    ...
    try {
        const buffer = (0, fs_1.readFileSync)(tempPath);
        if (!buffer.length) {
...
exports.scannerService = {
    ...
    async startScan(deviceId, settings) {
        ...
        return {
            ...
            settings,
            format: "jpeg",
            imageBuffer: imageBuffer.buffer.slice(imageBuffer.byteOffset, imageBuffer.byteOffset + imageBuffer.byteLength),
        };
    },
    async cancelScan() {
        return { success: true };
